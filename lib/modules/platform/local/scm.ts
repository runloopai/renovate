...
import { writeLocalFile, writeLocalSymlink } from '../../../util/fs/index.ts';
...
export class LocalFs implements PlatformScm {
  ...
  async commitAndPush(
    ...
    // Unlike the canonical committer (lib/util/git/index.ts), this doesn't
    // special-case directory paths as silently-ignored or track a separate
    // `ignoredFiles` list - Renovate managers aren't expected to produce a
    // directory path here, so a mismatch surfaces as a normal write/commit
    // failure instead.
    try {
      for (const file of files) {
        ...
        if (file.isSymlink) {
          await writeLocalSymlink(file.path, file.contents as string);
        } else if (file.contents === null) {
          // No content change to write - matches the canonical committer's
          // handling of `contents === null` in lib/util/git/index.ts.
          continue;
        } else {
      ...
      if (stagedPaths.length) {
        try {
          ...
          logger.warn(
            { err: resetErr },
            'local: failed to unstage files after a failed commit attempt',
          );
        }
      }
      logger.warn(
        { err, files: stagedPaths },
        'local: failed to create local commit; unstaged the affected files, but their contents on disk may still reflect this attempt - run `git status`/`git diff` before retrying',
      );
      return null;
    }
  }
